...
import com.example.djshichaoren.googleocrtest2.subtitle_api.parser.exception.InvalidAssSubException;
import com.example.djshichaoren.googleocrtest2.subtitle_api.subtitle.ass.ASSSub;
import com.example.djshichaoren.googleocrtest2.subtitle_api.subtitle.ass.ASSTime;
import com.example.djshichaoren.googleocrtest2.subtitle_api.subtitle.ass.Events;
import com.example.djshichaoren.googleocrtest2.subtitle_api.subtitle.ass.ScriptInfo;
import com.example.djshichaoren.googleocrtest2.subtitle_api.subtitle.ass.V4Style;
import com.example.djshichaoren.googleocrtest2.subtitle_api.utils.ColorUtils;
...
public class ASSParser extends BaseParser<ASSSub> {
    ...
    private static V4Style parseV4Style(String[] styleFormat, String[] styleLine, int lineIndex)
        ...
        for (int i = 0; i < styleFormat.length; i++) {
            ...
            String error = callProperty(V4Style.class, style, property, value);
    ...
    /**
     * Get the RGB code from the &HBBGGRR or &HAABBGGRR pattern
     *
    ...
    private static int getRGB(String value) {
        ...
        if (length == 10) {
            // From ASS
            rgb = ColorUtils.HAABBGGRRToRGB(value);
        } else if (length == 8) {
            // From SSA
            rgb = ColorUtils.HBBGGRRToRGB(value);
        }
        return rgb;
    }
...
//                    }
    ...
    private static String callProperty(Class<?> clazz, Object object, String property, String value) {
        String error = null;

        try {
//            System.out.println("clazz = [" + clazz + "], object = [" + object + "], property = [" + property + "], value = [" + value + "]");

            Field declaredField = clazz.getDeclaredField(property);
            Class<?> fieldType = declaredField.getType();

            Method declaredMethod = clazz.getDeclaredMethod("set" + StringUtils.capitalize(property), fieldType);
...
//                    PropertyUtils.setProperty(object, property, doubleValue);
            ...
            e.printStackTrace();
            System.out.println("clazz = [" + clazz + "], property = [" + property + "], value = [" + value + "]");
        }
