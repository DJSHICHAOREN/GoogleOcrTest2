...
import com.example.djshichaoren.googleocrtest2.subtitle_api.parser.exception.InvalidFileException;
import com.example.djshichaoren.googleocrtest2.subtitle_api.subtitle.common.TimedTextFile;
import com.example.djshichaoren.googleocrtest2.subtitle_api.utils.FileUtils;
...
public abstract class BaseParser<T extends TimedTextFile> implements SubtitleParser {
    ...
    @Override
    public T parse(File file, String encodingCharSet) {
        ...
        try (FileInputStream fis = new FileInputStream(file)) {
            return parse(fis, file.getName(), encodingCharSet);
        } catch (Exception e) {
    ...
    @Override
    public T parse(InputStream is, String fileName, String encodingCharSet) {
        ...
        try {
            ...
            try (InputStream nis = new ByteArrayInputStream(bytes);
                 InputStreamReader isr = new InputStreamReader(nis, encodingCharSet != null ? encodingCharSet : FileUtils.guessEncoding(bytes));
